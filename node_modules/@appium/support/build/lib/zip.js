...
async function extractAllTo(zipFilePath, destDir, opts = /** @type {ExtractAllOptions} */ ({})) {
    ...
    await fs_2.fs.mkdir(destDir, { recursive: true });
    const dir = await fs_2.fs.realpath(destDir);
...
async function _extractEntryTo(zipFile, entry, destDir) {
    ...
    if (/\/$/.test(entry.fileName)) {
        if (!(await fs_2.fs.exists(dstPath))) {
            await fs_2.fs.mkdirp(dstPath);
        }
        return;
    }
    else if (!(await fs_2.fs.exists(path_1.default.dirname(dstPath)))) {
        await fs_2.fs.mkdirp(path_1.default.dirname(dstPath));
    }
...
async function toInMemoryZip(srcPath, opts = /** @type {ZipOptions} */ ({})) {
    if (!(await fs_2.fs.exists(srcPath))) {
    ...
    if ((await fs_2.fs.stat(srcPath)).isDirectory()) {
...
async function assertValidZip(filePath) {
    if (!(await fs_2.fs.exists(filePath))) {
        throw new Error(`The file at '${filePath}' does not exist`);
    }
    const { size } = await fs_2.fs.stat(filePath);
    if (size < 4) {
        throw new Error(`The file at '${filePath}' is too small to be a ZIP archive`);
    }
    const fd = await fs_2.fs.open(filePath, 'r');
    try {
        const buffer = Buffer.alloc(ZIP_MAGIC.length);
        await fs_2.fs.read(fd, buffer, 0, ZIP_MAGIC.length, 0);
...
async function toArchive(dstPath, src = /** @type {ZipSourceOptions} */ ({}), opts = /** @type {ZipCompressionOptions} */ ({})) {
    ...
    const stream = fs_2.fs.createWriteStream(dstPath);
...
async function getExecutablePath(binaryName) {
    const fullPath = await fs_2.fs.which(binaryName);
