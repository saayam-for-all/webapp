    ...
    visitor: {
      CallExpression(path, state) {
        ...
        if (isGlobKey(propertyName) && t.isMetaProperty(callee.object) && args.length === 1 && t.isStringLiteral(args[0])) {
          ...
          const globPaths = (0, _glob.globSync)(args[0].value, {
            cwd,
            dotRelative: true
          }).sort().map(globPath => globPath.replace(/\\/g, '/'));
      ...
      VariableDeclaration(path, state) {
        ...
        if (isGlobKey(propertyName) && t.isMetaProperty(callee.object) && args.length === 2 && t.isStringLiteral(args[0]) && t.isObjectProperty(eagerOption[0]) && t.isBooleanLiteral(eagerOption[0].value)) {
          const cwd = _path.default.dirname(sourceFile);
          const globPaths = (0, _glob.globSync)(args[0].value, {
            cwd,
            dotRelative: true
          ...
          if (eagerOption[0].value.value) {
            ...
            const replacement = t.variableDeclaration('const', [t.variableDeclarator(identifier, t.objectExpression(globPaths.map(globPath => t.objectProperty(t.stringLiteral(globPath), asts[propertyName](globPath)))))]);
            path.replaceWith(replacement);
          }
        }
      }
